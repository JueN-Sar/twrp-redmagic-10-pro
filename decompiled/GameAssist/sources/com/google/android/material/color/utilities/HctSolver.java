...
public class HctSolver {
        ...
        for (int i2 = 0; i2 < 3; i2++) {
            ...
            if (d4 != d5) {
                ...
                for (int i5 = 0; i5 < 8 && Math.abs(i4 - i3) > 1; i5++) {
                    ...
                    double[] p2 = p(dArr, f14344d[floor], dArr2, i2);
                    double h3 = h(p2);
                    if (a(h2, d3, h3)) {
                        dArr2 = p2;
                        i4 = floor;
    ...
    static double d(double d2) {
        ...
        return ((MathUtils.g(d2) * 400.0d) * pow) / (pow + 27.13d);
    }
    ...
    static int g(double d2, double d3, double d4) {
        ...
        ViewingConditions viewingConditions = ViewingConditions.f14391k;
        double d5 = 1.0d;
        double pow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.f()), 0.73d);
        double cos = (Math.cos(d2 + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * viewingConditions.h() * viewingConditions.i();
        ...
        while (i2 < 5) {
            ...
            double b2 = (viewingConditions.b() * Math.pow(d6, (d5 / viewingConditions.c()) / viewingConditions.k())) / viewingConditions.g();
            double d8 = (((0.305d + b2) * 23.0d) * pow2) / (((23.0d * d7) + ((pow2 * 11.0d) * cos2)) + ((pow2 * 108.0d) * sin));
            double d9 = d8 * cos2;
            double d10 = d8 * sin;
            double d11 = b2 * 460.0d;
            double[] d12 = MathUtils.d(new double[]{j((((451.0d * d9) + d11) + (288.0d * d10)) / 1403.0d), j(((d11 - (891.0d * d9)) - (261.0d * d10)) / 1403.0d), j(((d11 - (d9 * 220.0d)) - (d10 * 6300.0d)) / 1403.0d)}, f14342b);
            double d13 = d12[0];
            if (d13 < 0.0d) {
                break;
            }
            ...
            if (d12[0] <= 100.01d && d12[1] <= 100.01d && d12[2] <= 100.01d) {
                return ColorUtils.a(d12);
            }
        }
        return 0;
    }
    ...
    static double h(double[] dArr) {
        double[] d2 = MathUtils.d(dArr, f14341a);
        double d3 = d(d2[0]);
        double d4 = d(d2[1]);
        double d5 = d(d2[2]);
        return Math.atan2(((d3 + d4) - (d5 * 2.0d)) / 9.0d, (((d3 * 11.0d) + ((-12.0d) * d4)) + d5) / 11.0d);
    }
    ...
    static double j(double d2) {
        ...
        return MathUtils.g(d2) * Math.pow(Math.max(0.0d, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }
    ...
    static double[] n(double d2, int i2) {
        ...
        if (i2 < 4) {
            ...
            return k(d8) ? new double[]{d8, d6, d7} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i2 < 8) {
            double d9 = ((d2 - (d3 * d7)) - (d5 * d6)) / d4;
            return k(d9) ? new double[]{d7, d9, d6} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d10 = ((d2 - (d3 * d6)) - (d4 * d7)) / d5;
        return k(d10) ? new double[]{d6, d7, d10} : new double[]{-1.0d, -1.0d, -1.0d};
    }
    ...
    public static int q(double d2, double d3, double d4) {
        if (d3 < 1.0E-4d || d4 < 1.0E-4d || d4 > 99.9999d) {
            return ColorUtils.b(d4);
        }
        double e2 = (MathUtils.e(d2) / 180.0d) * 3.141592653589793d;
        double p2 = ColorUtils.p(d4);
        int g2 = g(e2, d3, p2);
        return g2 != 0 ? g2 : ColorUtils.a(b(p2, e2));
    }
