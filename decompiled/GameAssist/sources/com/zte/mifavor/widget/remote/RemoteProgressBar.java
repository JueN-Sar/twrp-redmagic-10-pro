...
public class RemoteProgressBar extends ProgressBar {
    ...
    public RemoteProgressBar(@NonNull Context context, @Nullable AttributeSet attributeSet) {
        this(context, attributeSet, R.attr.colorProgressBarZTEStyle);
    }

    private Bitmap a(int[] iArr, int i2, int i3) {
        if (i2 <= 0) {
            Log.e("RemoteProgressBar", "build Bitmap width error, width = " + i2);
            i2 = Utils.c(this.mContext, 2);
        }
        if (i3 <= 0) {
            Log.e("RemoteProgressBar", "build Bitmap height error, height = " + i3);
        ...
        try {
            ...
            Log.e("RemoteProgressBar", "build Bitmap error, e = ", e2);
    ...
    private synchronized void c(int i2) {
        try {
            ...
            if (onProgressChangeListener != null) {
                ...
                Log.w("RemoteProgressBar", "doProgressRefresh error, doProgressRefresh is null.");
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    private float d(float f2) {
        try {
            ...
            Log.e("RemoteProgressBar", "format Float error, e = " + e2);
            return 0.0f;
        }
    }
    ...
    private float h() {
        ...
        if (this.DEBUG) {
            Log.e("RemoteProgressBar", "processProgress progress = " + f2);
        }
        return f2;
    }
        ...
        try {
            ...
            if (this.mPaint != null && canvas != null) {
                ...
                if (f2 < this.mProgress) {
                    this.mProgress = f2;
                    Log.d("RemoteProgressBar", "on Draw , mProgress = " + this.mProgress + ", mMax = " + this.mMax);
                }
                float f3 = this.mProgress;
                float f4 = this.mMin;
                if (f3 < f4) {
                    this.mProgress = f4;
                    Log.d("RemoteProgressBar", "on Draw , mProgress = " + this.mProgress + ", mMin = " + this.mMin);
                }
                this.mDelta = this.mMax - this.mMin;
                ...
                if (f7 > f8) {
                    ...
                    if (this.DEBUG) {
                        Log.d("RemoteProgressBar", "on Draw +++++ , mProgress = " + this.mProgress + ", mThumbCenterX = " + this.mThumbCenterX + ", width = " + i2 + ", mTrackSize = " + this.mTrackSize + ", mLeftPadding = " + this.mLeftPadding + ", mDelta = " + this.mDelta);
                    }
                    ...
                    Log.w("RemoteProgressBar", "on Draw +++++ , mProgress = " + this.mProgress + ", mThumbCenterX = " + this.mThumbCenterX);
                }
                if (this.mIsEnable) {
                    setAlpha(1.0f);
                } else {
    ...
    protected synchronized void onMeasure(int i2, int i3) {
        ...
        setMeasuredDimension(ProgressBar.resolveSize(Utils.c(this.mContext, 180), i2), getPaddingTop() + this.mTrackSize + getPaddingBottom());
    ...
    */
    public boolean onTouchEvent(android.view.MotionEvent r6) {
        ...
        L3a:
            ...
            java.lang.String r4 = "RemoteProgressBar"
        ...
        Le5:
            return r1
        */
        throw new UnsupportedOperationException("Method not decompiled: com.zte.mifavor.widget.remote.RemoteProgressBar.onTouchEvent(android.view.MotionEvent):boolean");
    }

    public void setColors(int[] iArr) {
        if (iArr == null || 2 != iArr.length) {
            Log.w("RemoteProgressBar", "set color, the colors is error.");
        } else {
    ...
    public void setOnProgressChangeListener(OnProgressChangeListener onProgressChangeListener) {
        ...
        Log.w("RemoteProgressBar", "set OnProgress Change Listener is " + onProgressChangeListener);
    }

    @Override // android.widget.ProgressBar
    public void setProgress(int i2) {
        if (this.DEBUG) {
            Log.d("RemoteProgressBar", "set Progress is " + i2 + ", max = " + this.mMax + ", min = " + this.mMin);
        }
        if (Math.abs(this.mMax - i2) < 0.001d) {
            i2 = (int) this.mMax;
        }
        if (Math.abs(this.mMin - i2) < 0.001d) {
            i2 = (int) this.mMin;
        }
        this.mProgress = i2;
        invalidate();
        if (this.DEBUG) {
            Log.w("RemoteProgressBar", "invalidate and set Progress is mProgress = " + this.mProgress);
        }
        c((int) this.mProgress);
    }

    public RemoteProgressBar(@NonNull Context context, @Nullable AttributeSet attributeSet, int i2) {
        super(context, attributeSet, i2);
        this.TAG = "RemoteProgressBar";
        this.DEBUG = true;
        this.mIsEnable = true;
        this.mViewWidth = 0;
        this.mViewHeight = 0;
        ...
        Log.d("RemoteProgressBar", "RemoteProgressBar in.");
        ...
        this.mTrackSize = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorProgressBarZTE_cpbz_track_size, Utils.c(context, 6));
        this.isTouchToSeek = obtainStyledAttributes.getBoolean(R.styleable.ColorProgressBarZTE_cpbz_touch_to_seek, true);
        this.mTrackBgColor = obtainStyledAttributes.getColor(R.styleable.ColorProgressBarZTE_cpbz_track_bg_color, Color.parseColor("#1A7D7D7D"));
        this.mColorsProgress = new int[]{obtainStyledAttributes.getColor(R.styleable.ColorProgressBarZTE_cpbz_colors_left, Color.parseColor("#0F87F7")), obtainStyledAttributes.getColor(R.styleable.ColorProgressBarZTE_cpbz_colors_right, Color.parseColor("#0F87F7"))};
        setEnabled(obtainStyledAttributes.getBoolean(R.styleable.ColorProgressBarZTE_android_enabled, isEnabled()));
        this.mTopLeftRadius = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorProgressBarZTE_cpbz_topLeftRadius, Utils.c(context, 6));
        this.mTopRightRadius = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorProgressBarZTE_cpbz_topRightRadius, Utils.c(context, 6));
        this.mBottomLeftRadius = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorProgressBarZTE_cpbz_bottomLeftRadius, Utils.c(context, 6));
        int dimensionPixelSize = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorProgressBarZTE_cpbz_bottomRightRadius, Utils.c(context, 6));
        ...
        if (this.DEBUG) {
            Log.d("RemoteProgressBar", "RemoteProgressBar out. mTrackSize = " + this.mTrackSize + ", mTopLeftRadius=" + this.mTopLeftRadius + ", mBottomRightRadius=" + this.mBottomRightRadius);
        }
    }
}
