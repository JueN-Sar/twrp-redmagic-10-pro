...
public class RemoteSeekBar extends SeekBar {
    ...
    public RemoteSeekBar(@NonNull Context context, @Nullable AttributeSet attributeSet) {
        this(context, attributeSet, R.attr.colorSeekBarZTEStyle);
    }

    private Bitmap l(int[] iArr, int i2, int i3) {
        if (i2 <= 0) {
            Log.e("RemoteSeekBar", "build Bitmap width error, width = " + i2);
            i2 = Utils.c(this.mContext, 2);
        }
        if (i3 <= 0) {
            Log.e("RemoteSeekBar", "build Bitmap height error, height = " + i3);
            i3 = Utils.c(this.mContext, 2);
        }
        try {
            ...
            Log.e("RemoteSeekBar", "build Bitmap error, e = ", e2);
            return Bitmap.createBitmap(Utils.c(this.mContext, 2), Utils.c(this.mContext, 2), Bitmap.Config.RGB_565);
        }
    }
    ...
    private float m() {
        ...
        if (this.DEBUG) {
            Log.d("RemoteSeekBar", "calculate Progress mThumbCenterX=" + this.mThumbCenterX + ", mSBLeft=" + this.mSBLeft + ", mDelta=" + this.mDelta + ", mTrackLength=" + this.mTrackLength + ", mMin=" + this.mMin + ", mMax=" + this.mMax + ", progress = " + f2);
        }
        return f2;
    }
    ...
    private float p(float f2) {
        try {
            ...
            Log.e("RemoteSeekBar", "format Float error, e = " + e2);
            return 0.0f;
        }
    }
    ...
    private void q() {
        ...
        if (this.DEBUG) {
            Log.d("RemoteSeekBar", "initConfigByPriority out, mMax = " + this.mMax + ", mMin = " + this.mMin + ", mDelta = " + this.mDelta + ", mProgress = " + this.mProgress + ", mPreProgress = " + this.mPreProgress);
        }
    }
    ...
    public void r(float f2, boolean z, boolean z2) {
        ...
        if (onSeekBarChangeListener != null && z2) {
            ...
            Log.d("RemoteSeekBar", "invalidate Progress. mOnSeekBarChangeListener=" + this.mOnSeekBarChangeListener + ", mIsRealtimeCallback = " + this.mIsRealtimeCallback);
        }
        int i2 = (int) f2;
        ...
        if (this.DEBUG) {
            Log.w("RemoteSeekBar", "+++++++++++ invalidate Progress out, mPreProgress=" + this.mPreProgress + ", isFromUser=" + z + ", isCallback=" + z2);
        }
    }
    ...
    private boolean s(MotionEvent motionEvent) {
        ...
        return ((motionEvent.getX() - f2) * (motionEvent.getX() - f2)) + ((motionEvent.getY() - measuredHeight) * (motionEvent.getY() - measuredHeight)) <= (this.mSBLeft + ((float) Utils.c(this.mContext, 8))) * (this.mSBLeft + ((float) Utils.c(this.mContext, 8)));
    }
    ...
    private void v(float f2, final float f3, int i2, boolean z) {
        float f4 = f2;
        if (this.DEBUG) {
            Log.d("RemoteSeekBar", "setProgressWithAnimation in, pre = " + f4 + ", post=" + f3 + ", duration=" + i2 + ", isCancelable=" + z + ", mIsCancellable=" + this.mIsCancellable + ", mProgressAnimation=" + this.mProgressAnimation + ", this=" + this);
        }
        if (!this.mIsCancellable) {
            if (this.DEBUG) {
                Log.w("RemoteSeekBar", "Progress Animation is Running and noncancelable. do nothing, pre=" + f4 + ", post=" + f3 + ", mProgress=" + this.mProgress + ", mIsCancellable=" + this.mIsCancellable);
                return;
            }
            return;
        }
        ...
        if (valueAnimator != null && valueAnimator.isRunning()) {
            if (f3 == this.mFinalPregress) {
                Log.w("RemoteSeekBar", "The Post and last post are the same, the Animation continues to execute, post=" + f3 + ", mFinalPregress=" + this.mFinalPregress + ", mIsCancellable=" + this.mIsCancellable);
                return;
            }
            this.mProgressAnimation.cancel();
            this.mProgressAnimation = null;
            if (this.DEBUG) {
                Log.w("RemoteSeekBar", "+++++++++++ Cancel Animation, pre=" + f4 + ", mProgress=" + this.mProgress + ", post=" + f3 + ", mIsCancellable=" + this.mIsCancellable + ", mFinalPregress=" + this.mFinalPregress);
            }
        ...
        if (this.DEBUG) {
            Log.w("RemoteSeekBar", "update mFinalPregress=" + this.mFinalPregress + ", pre=" + f4);
        }
        ValueAnimator valueAnimator2 = this.mProgressAnimation;
        if (valueAnimator2 == null || valueAnimator2.isRunning()) {
            if (this.DEBUG) {
                Log.w("RemoteSeekBar", "+++++++++++ setProgressWithAnimation warning, isRunning. pre=" + f4 + ", post=" + f3 + ", duration=" + i2);
                return;
            }
            return;
        }
        ...
        this.mProgressAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.zte.mifavor.widget.remote.RemoteSeekBar.1
            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public void onAnimationUpdate(ValueAnimator valueAnimator3) {
                float floatValue = ((Float) valueAnimator3.getAnimatedValue()).floatValue();
                if (RemoteSeekBar.this.DEBUG) {
                    Log.d("RemoteSeekBar", "+++++++++++ onAnimationUpdate, progress=" + floatValue + ", post=" + f3 + ", finalIsFromUser=" + z2 + ", mIsRealtimeCallback=" + RemoteSeekBar.this.mIsRealtimeCallback);
                }
                RemoteSeekBar remoteSeekBar = RemoteSeekBar.this;
                remoteSeekBar.r(floatValue, z2, remoteSeekBar.mIsRealtimeCallback);
                if (Math.abs(f3 - floatValue) < 0.01f) {
                    RemoteSeekBar.this.n(z2);
                    RemoteSeekBar.this.mIsCancellable = true;
                    RemoteSeekBar.this.mPreProgress = (int) floatValue;
                    if (RemoteSeekBar.this.mProgressAnimation != null) {
                        RemoteSeekBar.this.mProgressAnimation.cancel();
                    }
                    RemoteSeekBar.this.mProgressAnimation = null;
                    if (RemoteSeekBar.this.DEBUG) {
                        Log.w("RemoteSeekBar", "+++++++++++ End Progress Animation, mPreProgress=" + RemoteSeekBar.this.mPreProgress + ", mProgress=" + RemoteSeekBar.this.mProgress + ", mFinalPregress=" + RemoteSeekBar.this.mFinalPregress + ", mProgressAnimation=" + RemoteSeekBar.this.mProgressAnimation);
                    }
                }
            }
        });
        if (this.DEBUG) {
            Log.w("RemoteSeekBar", "+++++++++++ Start Progress Animation with pathInterpolator, pre=" + f4 + ", post=" + f3 + ", duration=" + i2 + ", mProgressAnimation=" + this.mProgressAnimation + ", mIsCancellable=" + this.mIsCancellable + ", this=" + this);
        }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        try {
            ...
            if (this.mPaint != null && canvas != null) {
                ...
                if (0.1d < f2 && f2 < progress) {
                    this.mProgress = f2;
                    Log.d("RemoteSeekBar", "on Draw , mProgress = " + this.mProgress + ", mDefinedMax = " + this.mDefinedMax);
                }
                float f3 = this.mProgress;
                float f4 = this.mDefinedMin;
                if (f3 < f4) {
                    this.mProgress = f4;
                    Log.d("RemoteSeekBar", "on Draw , mProgress = " + this.mProgress + ", mDefinedMin = " + this.mDefinedMin);
                }
                ...
                int i3 = this.mLength;
                if (i2 > i3) {
                    Log.d("RemoteSeekBar", "on Draw error, width = " + i3 + ", mLength = " + this.mLength);
                    i2 = i3;
                }
                canvas.drawBitmap(l(this.mColorsProgress, i2, this.mTrackSize), this.mLeftPadding, this.mTopPadding, this.mPaint);
                if (this.DEBUG) {
                    Log.d("RemoteSeekBar", "on Draw ++++++++++++++++++++++++++++++ mProgress=" + this.mProgress + ", mPreProgress=" + this.mPreProgress);
                }
    ...
    protected synchronized void onMeasure(int i2, int i3) {
        try {
            ...
            if (this.DEBUG) {
                Log.d("RemoteSeekBar", "on Measure in, mLeftPadding=" + this.mLeftPadding + ", mRightPadding=" + this.mRightPadding + ", widthMeasureSpec=" + i2 + ", heightMeasureSpec=" + i3 + ", mViewWidth=" + this.mViewWidth + ", mViewHeight=" + this.mViewHeight);
            }
            this.mViewWidth = getMeasuredWidth();
            ...
            if (this.DEBUG) {
                Log.d("RemoteSeekBar", "on Measure out, mViewWidth=" + this.mViewWidth + ", mLeftPadding=" + this.mLeftPadding + ", mRightPadding=" + this.mRightPadding + ", mTopPadding=" + this.mTopPadding + ", mBottomPadding=" + this.mBottomPadding + ", mSBLeft=" + this.mSBLeft + ", mSBRight=" + this.mSBRight + ", mTrackLength=" + this.mTrackLength + ", mLength=" + this.mLength);
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (actionMasked != 0) {
            if (actionMasked != 1) {
                if (actionMasked == 2) {
                    ...
                    if (this.DEBUG) {
                        ...
                        Log.d("RemoteSeekBar", sb.toString());
                    } else {
                        i3 = y;
                    }
                    if (this.isThumbOnDragging || t) {
                        ...
                        if (this.DEBUG) {
                            Log.w("RemoteSeekBar", "ACTION_MOVE mThumbCenterX=" + this.mThumbCenterX + ", mIsSliding = " + this.mIsSliding + ", dxx = " + this.dxx);
                        }
                        ...
                        if (z3) {
                            ...
                            if (m2 < this.mMinReachableProgress) {
                                if (this.DEBUG) {
                                    Log.d("RemoteSeekBar", "ACTION_MOVE, mProgress=" + this.mProgress + ", mMinReachableProgress=" + this.mMinReachableProgress + ", mPreProgress=" + this.mPreProgress);
                                }
                            ...
                            if (this.DEBUG) {
                                Log.d("RemoteSeekBar", "ACTION_MOVE, mThumbCenterX=" + this.mThumbCenterX + ", isThumbOnDragging=" + this.isThumbOnDragging + ", isTrackTouched=" + t + ", localPregress=" + progress + ", mProgress=" + this.mProgress);
                            }
                            ...
                            Log.d("RemoteSeekBar", "ACTION_MOVE isRunning=" + z3 + ", x=" + x + ", y=" + i3 + ", dx=" + i4 + ", dy=" + i5 + ", mInitialTouchX=" + this.mInitialTouchX + ", mInitialTouchY=" + this.mInitialTouchY + ", mTouchSlop=" + this.mTouchSlop);
                        }
                    } else {
                        if (this.DEBUG) {
                            Log.d("RemoteSeekBar", "ACTION_MOVE isThumbOnDragging=" + this.isThumbOnDragging + ", isTrackTouched = " + t + ", isTouchToSeek = " + this.isTouchToSeek + ", isRunning = " + z3 + ", mProgress = " + this.mProgress);
                        }
            ...
            if (this.DEBUG) {
                Log.d("RemoteSeekBar", "ACTION_UP or CANCEL, mThumbCenterX=" + this.mThumbCenterX + ", mIsSliding=" + this.mIsSliding);
            }
            ...
            if (m3 < this.mMinReachableProgress) {
                if (this.DEBUG) {
                    Log.d("RemoteSeekBar", "ACTION_UP or CANCEL, mProgress=" + this.mProgress + ", mMinReachableProgress=" + this.mMinReachableProgress + ", mPreProgress=" + this.mPreProgress);
                }
                this.mProgress = this.mMinReachableProgress;
            }
            float f10 = (this.mProgress - this.mPreProgress) / (this.mMax - this.mMin);
            if (this.DEBUG) {
                Log.d("RemoteSeekBar", "ACTION_UP or CANCEL, mThumbCenterX=" + this.mThumbCenterX + ", mIsSliding=" + this.mIsSliding + ", ratio=" + f10 + ", mProgress=" + this.mProgress + ", mPreProgress=" + this.mPreProgress + ", mIsUseAnimation = " + this.mIsUseAnimation + ", mFinalPregress=" + this.mFinalPregress);
            }
            if (this.mIsSliding && (i2 = this.mPreProgress) == 0 && f10 < 0.1f) {
                this.mProgress = i2;
                if (this.DEBUG) {
                    Log.w("RemoteSeekBar", "ACTION_UP or CANCEL, adjusted mProgress=" + this.mProgress + ", mPreProgress=" + this.mPreProgress + ", ratio=" + f10 + ", mIsSliding=" + this.mIsSliding);
                }
            }
            this.mFinalPregress = Math.round(this.mProgress);
            z = false;
            ...
            if (this.DEBUG) {
                Log.d("RemoteSeekBar", "ACTION_DOWN  mInitialTouchX=" + this.mInitialTouchX + ", mInitialTouchY=" + this.mInitialTouchY);
            }
            performClick();
            z2 = true;
            getParent().requestDisallowInterceptTouchEvent(true);
            if (this.isThumbOnDragging) {
                Log.d("RemoteSeekBar", "ACTION_DOWN  invalidate ...");
            ...
            if (this.DEBUG) {
                Log.d("RemoteSeekBar", "ACTION_DOWN, mThumbCenterX=" + this.mThumbCenterX + ", dxx=" + this.dxx + ", isTrackTouched=" + t + ", isThumbOnDragging=" + this.isThumbOnDragging + ", mProgress=" + this.mProgress + ", mPreProgress=" + this.mPreProgress);
            }
    ...
    public void setColors(int[] iArr) {
        if (iArr == null || 2 != iArr.length) {
            Log.w("RemoteSeekBar", "set color, the colors is error.");
        } else {
    ...
    public void setProgress(int i2) {
        if (this.DEBUG) {
            Log.d("RemoteSeekBar", "set Progress in, progress=" + i2 + ", mProgress=" + this.mProgress + ", mIsSliding=" + this.mIsSliding + ", mIsUseAnimation=" + this.mIsUseAnimation + ", mPreProgress=" + this.mPreProgress);
        }
        if (this.mIsSliding) {
            return;
        }
        float f2 = i2;
        if (Math.abs(f2 - this.mProgress) < 1.0E-5f) {
            return;
        }
        if (this.mIsUseAnimation) {
            ...
            if (valueAnimator != null && valueAnimator.isRunning()) {
                ...
                this.mProgressAnimation = null;
                if (this.DEBUG) {
                    Log.d("RemoteSeekBar", "cancel mProgressAnimation and invalidate Progress. mFinalPregress=" + this.mFinalPregress);
                }
                r(this.mFinalPregress, false, true);
            }
            r(f2, false, true);
        ...
        if (this.DEBUG) {
            Log.d("RemoteSeekBar", "set Progress out, mFinalPregress=" + this.mFinalPregress);
        }
    }
    ...
    public RemoteSeekBar(@NonNull Context context, @Nullable AttributeSet attributeSet, int i2) {
        super(context, attributeSet, i2);
        this.TAG = "RemoteSeekBar";
        this.DEBUG = true;
        ...
        if (this.DEBUG) {
            Log.d("RemoteSeekBar", "RemoteSeekBar in, mProgress=" + this.mProgress + ", mPreProgress=" + this.mPreProgress);
        }
        ...
        this.mTrackSize = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorSeekBarZTE_csbz_track_size, Utils.c(context, 20));
        this.mThumbRadius = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorSeekBarZTE_csbz_thumb_radius, Utils.c(context, 10));
        this.mThumbInnerRadius = obtainStyledAttributes.getDimensionPixelSize(R.styleable.ColorSeekBarZTE_csbz_thumb_inner_radius, Utils.c(context, 6));
        ...
        this.mTrackBgColor = obtainStyledAttributes.getColor(R.styleable.ColorSeekBarZTE_csbz_track_bg_color, Color.parseColor("#1A7D7D7D"));
        this.mColorsProgress = new int[]{obtainStyledAttributes.getColor(R.styleable.ColorSeekBarZTE_csbz_colors_left, Color.parseColor("#0F87F7")), obtainStyledAttributes.getColor(R.styleable.ColorSeekBarZTE_csbz_colors_right, Color.parseColor("#0F87F7"))};
