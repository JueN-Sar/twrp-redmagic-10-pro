...
public class OwlSysHelper {
    ...
    private void insertOwl_GameSpaceRedmagicTime_init() {
        ...
        insertOwlDayCv("cn.nubia.gamelauncher", "redmagic_time_YXLM_switch_status", "switch_status video_quality full_video death_video live_death_video", (Settings.Global.getInt(mCtx.getContentResolver(), "persist_sys_nubia_redmagic_time_switch_lol", 0) == 1 ? "on" : "off") + (Settings.Global.getInt(mCtx.getContentResolver(), "db_game_video_quality_lol", 0) == 1 ? " HD" : " SD") + (Settings.Global.getInt(mCtx.getContentResolver(), "persist_sys_nubia_full_video_switch_lol", 0) == 1 ? " on" : " off") + (Settings.Global.getInt(mCtx.getContentResolver(), "persist_sys_nubia_death_video_switch_lol", 1) == 1 ? " on" : " off") + (Settings.Global.getInt(mCtx.getContentResolver(), "persist_sys_nubia_real_time_death_switch_lol", 1) == 1 ? " on" : " off"));
    ...
    private void insertOwl_persCenter_init() {
        ...
        insertOwlDayCv("cn.nubia.gamelauncher", "game_mis_operate_switch", "switch_status option", "on ".concat((Build.DEVICE == null || !(Build.DEVICE.contains("NX629") || Build.DEVICE.contains("NX651") || Utils.isZte(mCtx))) ? (Build.DEVICE == null || !Build.DEVICE.contains("NX666")) ? "edge" : "both" : "game_cc"));
