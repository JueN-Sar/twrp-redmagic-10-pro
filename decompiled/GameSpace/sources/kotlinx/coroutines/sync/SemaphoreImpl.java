...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        ...
        loop0: while (true) {
            ...
            while (true) {
                if (segment2.getId() >= j && !segment2.getRemoved()) {
                    m2004constructorimpl = SegmentOrClosed.m2004constructorimpl(segment2);
                    break;
                }
                Object obj = segment2.get_next();
                if (obj == ConcurrentLinkedListKt.CLOSED) {
                    m2004constructorimpl = SegmentOrClosed.m2004constructorimpl(ConcurrentLinkedListKt.CLOSED);
                    break;
                }
            ...
            if (!SegmentOrClosed.m2009isClosedimpl(m2004constructorimpl)) {
                Segment m2007getSegmentimpl = SegmentOrClosed.m2007getSegmentimpl(m2004constructorimpl);
                while (true) {
                    Segment segment4 = (Segment) this.tail;
                    if (segment4.getId() >= m2007getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!m2007getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (tail$FU.compareAndSet(this, segment4, m2007getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m2007getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m2007getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m2007getSegmentimpl(m2004constructorimpl);
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m2009isClosedimpl(m2004constructorimpl)) {
                ...
                while (true) {
                    ...
                    if (head$FU.compareAndSet(this, segment4, m2007getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m2007getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
