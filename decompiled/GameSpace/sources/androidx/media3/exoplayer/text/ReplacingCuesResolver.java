...
final class ReplacingCuesResolver implements CuesResolver {
    ...
    public ImmutableList<Cue> getCuesAtTimeUs(long j) {
        ...
        return (cuesWithTiming.endTimeUs == C.TIME_UNSET || j < cuesWithTiming.endTimeUs) ? cuesWithTiming.cues : ImmutableList.of();
    }
    ...
    public long getNextCueChangeTimeUs(long j) {
        ...
        for (int i = 1; i < this.cuesWithTimingList.size(); i++) {
            ...
            if (j < cuesWithTiming.startTimeUs) {
                ...
                return (cuesWithTiming2.endTimeUs == C.TIME_UNSET || cuesWithTiming2.endTimeUs <= j || cuesWithTiming2.endTimeUs >= cuesWithTiming.startTimeUs) ? cuesWithTiming.startTimeUs : cuesWithTiming2.endTimeUs;
            }
        }
    ...
    public long getPreviousCueChangeTimeUs(long j) {
        ...
        for (int i = 1; i < this.cuesWithTimingList.size(); i++) {
            ...
            if (j < j2) {
                ...
                return (cuesWithTiming.endTimeUs == C.TIME_UNSET || cuesWithTiming.endTimeUs > j) ? cuesWithTiming.startTimeUs : cuesWithTiming.endTimeUs;
            }
        }
        CuesWithTiming cuesWithTiming2 = (CuesWithTiming) Iterables.getLast(this.cuesWithTimingList);
        return (cuesWithTiming2.endTimeUs == C.TIME_UNSET || j < cuesWithTiming2.endTimeUs) ? cuesWithTiming2.startTimeUs : cuesWithTiming2.endTimeUs;
    }
}
