...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    private static List<MediaCodecInfo> getDecoderInfos(MediaCodecSelector mediaCodecSelector, Format format, boolean z, AudioSink audioSink) throws MediaCodecUtil.DecoderQueryException {
        ...
        return format.sampleMimeType == null ? ImmutableList.of() : (!audioSink.supportsFormat(format) || (decryptOnlyDecoderInfo = MediaCodecUtil.getDecryptOnlyDecoderInfo()) == null) ? MediaCodecUtil.getDecoderInfosSoftMatch(mediaCodecSelector, format, z, false) : ImmutableList.of(decryptOnlyDecoderInfo);
    }
    ...
    public long getDurationToProgressUs(boolean z, long j, long j2) {
        ...
        return Math.max(Renderer.DEFAULT_DURATION_TO_PROGRESS_US, j3);
    }
    ...
    protected int supportsFormat(MediaCodecSelector mediaCodecSelector, Format format) throws MediaCodecUtil.DecoderQueryException {
        ...
        if ((!MimeTypes.AUDIO_RAW.equals(format.sampleMimeType) || this.audioSink.supportsFormat(format)) && this.audioSink.supportsFormat(Util.getPcmFormat(2, format.channelCount, format.sampleRate))) {
